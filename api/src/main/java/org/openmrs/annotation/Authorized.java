 ...
 * {@link org.openmrs.security.OpenmrsMethodSecurityConfig}), evaluated by a separate, independent
 * advisor alongside {@link org.openmrs.aop.AuthorizationAdvice} - a method carries one annotation
 * or the other, never both. {@code @Authorized} remains fully supported and is not going away;
 * there is no need to convert existing usages. New code, and contributors who prefer Spring
 * Security's standard mechanism, may use
 * {@code @PreAuthorize("hasPermission(null, '&lt;privilege&gt;')")} instead - see
